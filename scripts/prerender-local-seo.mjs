...
const mainTitle = 'Commerce Coaching in Mehsana — Class 11 & 12 CBSE';
const mainDescription = 'Commerce coaching in Mehsana for CBSE Class 11 and 12. Learn Economics, Business Studies and Accountancy with chapter-wise notes, tests, doubt solving and demo-class booking by Smit Sir Commerce.';
const mainFaqs = [
...
const mainStructuredData = {
  ...
  '@graph': [
    { '@type': 'WebPage', '@id': `${BASE}${mainPath}#webpage`, url: `${BASE}${mainPath}`, name: 'Commerce Coaching in Mehsana for Class 11 & 12', description: mainDescription, inLanguage: 'en-IN', about: { '@id': `${BASE}${mainPath}#service` }, isPartOf: { '@id': `${BASE}/#website` } },
    { '@type': 'Service', '@id': `${BASE}${mainPath}#service`, name: 'Class 11 & 12 Commerce Coaching in Mehsana', serviceType: 'CBSE Class 11 and Class 12 Commerce coaching', provider: { '@id': `${BASE}/#organization` }, areaServed: { '@type': 'City', name: 'Mehsana' }, offers: { '@type': 'Offer', url: `${BASE}/book-demo`, description: 'Request a demo class before joining a Commerce batch.' } },
    { '@type': 'FAQPage', mainEntity: mainFaqs.map(([question, answer]) => ({ '@type': 'Question', name: question, acceptedAnswer: { '@type': 'Answer', text: answer } })) },
    { '@type': 'BreadcrumbList', itemListElement: [{ '@type': 'ListItem', position: 1, name: 'Home', item: `${BASE}/` }, { '@type': 'ListItem', position: 2, name: 'Commerce Coaching in Mehsana', item: `${BASE}${mainPath}` }] },
  ],
};
const mainBody = `<main class="page-container section-padding" data-prerendered="local-seo"><nav aria-label="Breadcrumb"><a href="/">Home</a> / Commerce Coaching in Mehsana</nav><article><p>Mehsana, Gujarat · CBSE Class 11 &amp; 12 Commerce</p><h1>Commerce Coaching in Mehsana for Class 11 &amp; 12</h1><p>CBSE Commerce learning focused on concept clarity, exam-style practice and chapter-wise revision. Smit Sir Commerce supports Class 11 and Class 12 students studying Economics, Business Studies, Accountancy and related Commerce subjects.</p><p><a href="/book-demo">Book a demo class</a> · <a href="/cbse-notes">Free CBSE notes</a></p><section><h2>Explore Commerce tuition options in Mehsana</h2><ul>${localSeoPages.map((page) => `<li><a href="${esc(page.path)}">${esc(page.h1)}</a></li>`).join('')}</ul></section><section><h2>Frequently asked questions</h2>${mainFaqs.map(([q, a]) => `<h3>${esc(q)}</h3><p>${esc(a)}</p>`).join('')}</section></article></main>`;
await writeRoute(mainPath, makeHtml({ path: mainPath, title: mainTitle, description: mainDescription, body: mainBody, structuredData: mainStructuredData, imageAlt: 'Commerce Coaching in Mehsana — Smit Sir Commerce' }));
...
for (const page of localSeoPages) {
  const structuredData = {
    ...
    '@graph': [
      ...
      { '@type': 'Service', '@id': `${BASE}${page.path}#service`, name: page.serviceName, serviceType: page.serviceType, provider: { '@id': `${BASE}/#organization` }, areaServed: { '@type': 'City', name: 'Mehsana', containedInPlace: { '@type': 'State', name: 'Gujarat' } }, offers: { '@type': 'Offer', url: `${BASE}/book-demo`, description: 'Request a demo class before choosing a Commerce batch.' } },
      { '@type': 'FAQPage', mainEntity: page.faqs.map(([question, answer]) => ({ '@type': 'Question', name: question, acceptedAnswer: { '@type': 'Answer', text: answer } })) },
      { '@type': 'BreadcrumbList', itemListElement: [{ '@type': 'ListItem', position: 1, name: 'Home', item: `${BASE}/` }, { '@type': 'ListItem', position: 2, name: 'Commerce Coaching in Mehsana', item: `${BASE}${mainPath}` }, { '@type': 'ListItem', position: 3, name: page.h1, item: `${BASE}${page.path}` }] },
    ],
  };
  const related = localSeoPages.filter((item) => item.path !== page.path);
  const body = `<main class="page-container section-padding" data-prerendered="local-keyword-seo"><nav aria-label="Breadcrumb"><a href="/">Home</a> / <a href="${mainPath}">Commerce Coaching in Mehsana</a> / ${esc(page.h1)}</nav><article><p>${esc(page.eyebrow)}</p><h1>${esc(page.h1)}</h1><p>${esc(page.intro)}</p><p><a href="/book-demo">Book a demo class</a> · <a href="/cbse-notes">Free CBSE Commerce notes</a></p><section><h2>${esc(page.sectionTitle)}</h2><p>${esc(page.sectionText)}</p><ul>${page.focus.map(([title, text]) => `<li><strong>${esc(title)}:</strong> ${esc(text)}</li>`).join('')}</ul></section><section><h2>Questions students and parents ask</h2>${page.faqs.map(([q, a]) => `<h3>${esc(q)}</h3><p>${esc(a)}</p>`).join('')}</section><section><h2>Related Commerce tuition in Mehsana</h2><ul>${related.map((item) => `<li><a href="${esc(item.path)}">${esc(item.h1)}</a></li>`).join('')}</ul></section><p><a href="/study-material">Study material</a> · <a href="/test-series">Commerce tests</a> · <a href="/contact">Contact Smit Sir Commerce</a></p></article></main>`;
...
console.log(`Pre-rendered Mehsana local Commerce coaching landing page plus ${localSeoPages.length} local keyword pages.`);
