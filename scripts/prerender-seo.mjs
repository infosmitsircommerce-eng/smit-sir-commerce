...
function renderGsebMaterial(material) {
  const details = gsebChapterDetails[material.id] || {
    summary: material.description || `Revise ${material.chapter} with organised explanations, examples and exam-focused notes prepared for GSEB Commerce students.`,
    topics: material.keyTopics || [material.chapter, 'Important definitions', 'Concept explanations', 'Examples and applications', 'Exam-focused revision'],
  };
  ...
  const accountancyGuide = material.subject === 'Accountancy'
    ? '<ol><li>Read the concept and understand why each account or book is used.</li><li>Revise the format, rules and key terminology before written practice.</li><li>Close the PDF and recall the main headings in your own words.</li><li>Use this free concept edition first; keep detailed numerical practice for the separate Premium edition.</li><li>Return to weak sections, then continue to the next available chapter.</li></ol>'
    : '<ol><li>Read the concept explanation and connect every definition with its example.</li><li>Underline the exact headings required for short and long board answers.</li><li>Revise comparisons in a basis-wise format so the differences remain clear.</li><li>Close the PDF and recall the main headings before beginning written practice.</li><li>Return to weak sections and then continue to the next chapter.</li></ol>';
  const editionNote = material.numericalsIncluded === false
    ? '<p><strong>Edition:</strong> Free Simple Explanation Notes - without a separate numerical practice set. Detailed numericals will be published later as a separate Premium collection, with Chapters 1 and 2 available as free demos.</p>'
    : '';
  const collectionPath = material.subject === 'Accountancy' && material.class_level === 11
    ? '/gseb-class-11-accountancy-notes'
    : `/study-material?board=GSEB&class=${material.class_level}&subject=${encodeURIComponent(material.subject)}`;

  return `<main class="page-container section-padding" data-prerendered="gseb-chapter"><nav aria-label="Breadcrumb"><a href="/">Home</a> / <a href="/study-material?board=GSEB">GSEB Study Material</a> / Class ${material.class_level} ${escapeHtml(subject)} / Chapter ${material.chapterNumber}</nav><article><h1>${escapeHtml(material.chapter)} Notes PDF - GSEB Class ${material.class_level} ${escapeHtml(subject)}</h1><p>${escapeHtml(details.summary)}</p><p><strong>Board:</strong> GSEB · <strong>Medium:</strong> English · <strong>Class:</strong> ${material.class_level} · <strong>Subject:</strong> ${escapeHtml(subject)} · <strong>PDF:</strong> ${material.pages} pages · <strong>Access:</strong> Free</p>${editionNote}<h2>What these chapter notes cover</h2><ul>${details.topics.map((topic) => `<li>${escapeHtml(topic)}</li>`).join('')}</ul><h2>How to study this ${escapeHtml(subject)} chapter</h2>${accountancyGuide}<h2>Free chapter PDF</h2><p><a href="${escapeHtml(material.file_url)}">View or download the free ${escapeHtml(material.chapter)} notes PDF</a>. The document is arranged for clear reading on mobile and printing on A4 paper.</p><h2>Prepared for focused GSEB Commerce revision</h2><p>These original chapter-wise notes are prepared by Smit Thaker for students who want straightforward explanations rather than scattered material. Use the PDF with your current school textbook and teacher guidance. Definitions, examples and answer-writing points should be revised together because board questions may test both memory and application.</p><h2>Frequently asked questions</h2><h3>Is this GSEB Class ${material.class_level} ${escapeHtml(subject)} PDF free?</h3><p>Yes. The available chapter PDF can be opened or downloaded without registration.</p><h3>Can I study this chapter on a mobile phone?</h3><p>Yes. Open the chapter page, use the PDF viewer or download the file for later revision.</p><h3>Are more GSEB ${escapeHtml(subject)} chapters being added?</h3><p>Yes. Only completed and checked chapters are marked as available; unfinished chapters are not shown as downloads.</p><h2>Continue studying</h2><ul>${previous ? `<li><a href="${escapeHtml(previous.seo_path)}">Previous: Chapter ${previous.chapterNumber} - ${escapeHtml(previous.chapter)}</a></li>` : ''}${next ? `<li><a href="${escapeHtml(next.seo_path)}">Next: Chapter ${next.chapterNumber} - ${escapeHtml(next.chapter)}</a></li>` : ''}<li><a href="${escapeHtml(collectionPath)}">Browse all GSEB Class ${material.class_level} ${escapeHtml(subject)} notes</a></li><li><a href="/quizzes">Practise with Commerce quizzes</a></li></ul></article></main>`;
}
...
function getGsebMaterialStructuredData(material) {
  ...
  return {
    ...
    name: `${material.chapter} Notes PDF - GSEB Class ${material.class_level} ${subject}`,
