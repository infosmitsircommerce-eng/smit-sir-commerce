...
function replaceRoot(html, body) {
  ...
  if (/<div id="root">[\s\S]*?<\/div>\s*(?=<\/body>)/.test(html)) {
    return html.replace(/<div id="root">[\s\S]*?<\/div>\s*(?=<\/body>)/, nextRoot);
  }
...
console.log(`Finalized ${pages.length} AdSense policy pages with correct visible bodies and canonicals.`);
