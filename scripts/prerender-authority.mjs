...
function body(guide) {
  ...
  const hubPath = guide.hubPath || '/cbse-practice';
  const hubLabel = guide.hubLabel || 'CBSE Practice';
  return `<main class="page-container section-padding" data-prerendered="authority-guide"><nav aria-label="Breadcrumb"><a href="/">Home</a> / <a href="${esc(hubPath)}">${esc(hubLabel)}</a> / ${esc(guide.shortTitle)}</nav><article><h1>${esc(guide.title)}</h1><p>${esc(guide.intro)}</p><p><strong>Prepared by Smit Sir</strong> · Updated ${esc(guide.updated)} · Free revision resource${enhancement.yearLabel ? ` · ${esc(enhancement.yearLabel)}` : ''}</p>${enhancementBody(guide)}${guide.sections.map(section=>`<section><h2>${esc(section.title)}</h2><p>${esc(section.text)}</p>${section.items?.length ? `<ol>${section.items.map(item=>`<li>${esc(item)}</li>`).join('')}</ol>` : ''}<ul>${section.links.map(([path,label])=>`<li><a href="${esc(path)}">${esc(label)}</a></li>`).join('')}</ul></section>`).join('')}<h2>Frequently asked questions</h2>${guide.faqs.map(([q,a])=>`<h3>${esc(q)}</h3><p>${esc(a)}</p>`).join('')}<h2>Editorial transparency</h2><p>This guide links to material actually published on Smit Sir Commerce. Original practice is not presented as an official board paper. Students should use current school or board-prescribed material for final syllabus and exam requirements.</p><p><a href="/about">About Smit Sir</a> · <a href="${esc(hubPath)}">${esc(hubLabel)}</a> · <a href="/study-material">Study Material</a> · <a href="/book-demo">Free Paper Analysis</a></p></article></main>`;
}
