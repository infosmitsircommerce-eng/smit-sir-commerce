...
function stripInteractiveApp(html) {
  return html
    .replace(/<script\s+type="module"[\s\S]*?<\/script>/g, '')
    .replace(/<link\s+rel="modulepreload"[\s\S]*?>/g, '')
    .replace(/<script\s+id="vite-plugin-pwa:register-sw"[\s\S]*?<\/script>/g, '')
    .replace(/<script\s+src="\/registerSW\.js"[\s\S]*?<\/script>/g, '')
    .replace(/<div id="app-startup-mask"[\s\S]*?<\/div>/g, '');
}
...
function renderPage(page) {
  ...
  const body = `<main class="page-container section-padding" data-prerendered="genuine-traffic"><article><p style="font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:#966313">Free Commerce Resource</p><h1>${esc(page.h1)}</h1><p><strong>Best for:</strong> ${esc(page.intent)}</p><p>${esc(page.intro)}</p><section><h2>Fastest useful start</h2><p>${esc(page.quickWin)}</p></section>${page.sections.map(renderSection).join('')}${renderLinks(page.links)}${renderFaq(page.faq)}<section><h2>Use this with the main website</h2><p>This page is a free study route, not an official board publication. For more learning support, open <a href="/study-material">Study Material</a>, <a href="/cbse-notes">CBSE Notes</a>, <a href="/gseb-class-12-economics.html">GSEB Economics PDFs</a>, <a href="/tools">Commerce Tools</a> and <a href="/games">Commerce Games</a>.</p></section></article></main>`;

  const html = stripInteractiveApp(source)
