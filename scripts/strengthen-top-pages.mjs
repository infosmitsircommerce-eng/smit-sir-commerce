...
const topPages = [
  {
    ...
    bullets: [
      'CBSE Class 11 and Class 12 Commerce material in one connected flow.',
      'GSEB Class 12 Economics notes and PDFs connected chapter-wise.',
      'A clear study path: read notes, download PDF, practise questions, use tools and revise again.',
      'Useful for students who do not want scattered files or demo-first pages.'
    ],
    faqs: [
  ...
  },
  {
    ...
    bullets: [
      'Class 11 Microeconomics notes and revision resources.',
      'Class 12 Business Studies and Macroeconomics notes where published.',
      'Chapter-first structure so students can open the exact notes page they need.',
      'Internal links to CBSE practice, PYQ preparation, calculators and study material.'
    ],
    faqs: [
  ...
  },
  {
    ...
    bullets: [
      'Chapter 2 to Chapter 11 Economics notes are listed separately.',
      'Each chapter connects to its own notes page and revision-practice section.',
      'Chapter 1 is not shown because it was not part of the uploaded published set.',
      'Useful for searches like GSEB Class 12 Economics notes PDF and Gujarat Board Economics notes.'
    ],
    faqs: [
  ...
  },
  {
    ...
    bullets: [
      'Economics calculators for national income, income determination, elasticity, revenue and cost topics.',
      'Accountancy calculators for ratios, common-size statements and related numerical support.',
      'Formula-first structure suitable for Class 11 and Class 12 Commerce revision.',
      'Internal links connect tools back to notes, practice and chapter learning.'
    ],
    faqs: [
  ...
  },
  {
    ...
    bullets: [
      'Local Mehsana context for Commerce tuition and student support.',
      'Clear subject focus around Economics, Business Studies, Entrepreneurship and related resources.',
      'Free notes and tools reduce pressure before students or parents ask for demo support.',
      'Linked to CBSE notes, study material, tools and contact pages for trust and navigation.'
    ],
    faqs: [
...
function candidateFiles(path) {
  ...
  const withoutHtml = clean.endsWith('.html') ? clean.slice(0, -5) : clean;
  const files = [join(distRoot, clean), join(distRoot, `${clean}.html`), join(distRoot, withoutHtml, 'index.html')];
...
function buildSection(page) {
  ...
  return `
<section data-top-page-seo-geo="${esc(page.path)}" aria-labelledby="top-page-seo-${esc(page.path).replace(/[^a-z0-9]+/gi, '-')}" style="margin-top:32px;padding:24px;border:1px solid rgba(183,121,31,.22);border-radius:24px;background:linear-gradient(135deg,#fffaf0,#ffffff);box-shadow:0 18px 42px rgba(23,19,15,.06)">
  <p style="margin:0 0 10px;color:#966313;font-size:12px;font-weight:900;letter-spacing:.12em;text-transform:uppercase">SEO + AI answer block</p>
  <h2 id="top-page-seo-${esc(page.path).replace(/[^a-z0-9]+/gi, '-')}" style="margin:0 0 12px">${esc(page.heading)}</h2>
  <p style="color:#5b6472;line-height:1.75">${esc(page.intro)}</p>
  <h3>What this page is best for</h3>
  <p style="color:#5b6472;line-height:1.75">${esc(page.intent)}</p>
  <ul>${page.bullets.map((item) => `<li>${esc(item)}</li>`).join('')}</ul>
  <h3>Connected Commerce resource pillars</h3>
  <ul>${related.map(([path, label]) => `<li><a href="${esc(path)}">${esc(label)}</a></li>`).join('')}</ul>
  <h3>Quick FAQ</h3>
  ${page.faqs.map(([q, a]) => `<details><summary><strong>${esc(q)}</strong></summary><p>${esc(a)}</p></details>`).join('')}
</section>`;
}
...
function patchHead(html, page) {
  ...
  if (!output.includes('href="https://www.smitsircommerce.in/llms.txt"') && !output.includes('href="/llms.txt"')) {
    output = output.replace(/<\/head>/i, `${aiLinks}\n</head>`);
  }
  const marker = `data-top-page-schema="${page.path}"`;
  if (!output.includes(marker)) {
    const schema = JSON.stringify(buildSchema(page)).replaceAll('<', '\\u003c');
    output = output.replace(/<\/head>/i, `<script type="application/ld+json" ${marker}>${schema}</script>\n</head>`);
  }
  return output;
}
...
for (const page of topPages) {
  for (const file of candidateFiles(page.path)) {
    ...
    let html = patchHead(before, page);
    html = patchBody(html, page);
    if (html !== before) {
...
for (const page of topPages) {
  const files = await Promise.all(candidateFiles(page.path).map(exists));
  if (!files.some(Boolean)) missing += 1;
}
