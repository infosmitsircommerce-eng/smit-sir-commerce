...
async function syncLibraryStats() {
  ...
  for (const match of html.matchAll(/\{u:(\d+),title:'[^']*',pages:(\d+)/g)) {
    unitPages.set(Number(match[1]), Number(match[2]));
  }
  ...
  if (unitPages.size === 0) {
    throw new Error('No NET/GSET unit page totals found while generating platform stats.');
  }

  const studyPageCount = [...unitPages.values()].reduce((sum, pages) => sum + pages, 0);
  const generated = `// Generated by scripts/sync-gset-pdfs.mjs during prebuild.\n// Keeping the generated values in source also lets local development start before prebuild runs.\nexport const netGsetFreePdfCount = ${chapterKeys.size};\nexport const netGsetStudyPageCount = ${studyPageCount};\nexport const netGsetLiveUnitCount = ${unitPages.size};\n`;
  ...
  console.log(`[gset-stats] ${chapterKeys.size} PDFs · ${studyPageCount} pages · ${unitPages.size} live units`);
}
