...
export default async function handler(req, res) {
  ...
  if (!enforceRateLimit(req, res, { bucket: 'enquiry', limit: 5, windowMs: 10 * 60_000 })) return;
  ...
  const isTeacherService =
    landingContext.startsWith('teacher-service:') ||
    intent === 'Teacher Studio Request';

  const text = [
    'New enquiry received on Smit Sir Commerce',
    '',
    `${isTeacherService ? 'Teacher / client' : 'Student'}: ${fullName}`,
    `Mobile: ${mobile}`,
    ...
    landingContext
      ? `${isTeacherService ? 'Service tracking' : 'Local landing context'}: ${landingContext}`
      : null,
    '',
  ...
  try {
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from:
          ...
          'Smit Sir Commerce <onboarding@resend.dev>',
        to: [process.env.ENQUIRY_TO_EMAIL || 'infosmitsircommerce@gmail.com'],
        subject: `${isTeacherService ? 'New Teacher Studio request' : 'New website enquiry'} — ${fullName}`,
        text,
      }),
    });
    ...
    console.error('Enquiry email error:', error?.message || 'unknown');
