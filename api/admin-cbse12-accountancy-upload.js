...
async function readRawBody(req) {
  ...
  for await (const chunk of req) {
    ...
    total += value.length;
    if (total > MAX_BYTES) throw new Error('This PDF is larger than the 4 MB secure-upload limit.');
    chunks.push(value);
  }
...
export default async function handler(req, res) {
  ...
  if (!resource) return res.status(404).json({ error: 'Unknown Accountancy resource.' });
  ...
  try {
    ...
    if (pdf.subarray(0, 5).toString() !== '%PDF-') return res.status(400).json({ error: 'Please upload the matching PDF file.' });
    ...
    return res.status(200).json({ ok: true, resourceKey, title: resource.title, pages: resource.pages, bytes: pdf.length, chunks: payloads.length, sha256 });
  } catch (error) {
    console.error('admin-cbse12-accountancy-upload', resourceKey, error?.message || error);
    return res.status(503).json({ error: error?.message || 'Unable to store this Premium chapter.' });
  }
}
