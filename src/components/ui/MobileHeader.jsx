...
export default function MobileHeader() {
  ...
  return (
    <>
      <header
        className="mobile-site-header lg:hidden sticky top-0 z-[60] flex items-center justify-between px-4 h-16"
        style={{
          background: light ? "rgba(247,248,252,0.96)" : "rgba(16,24,40,0.96)",
      ...
      >
        ...
        >
          ...
          <div className="min-w-0">
            ...
            <span
              className="font-bold text-[10px] block leading-none tracking-[0.16em] mt-1"
        ...
        <div className="flex items-center gap-2">
          <Link
            ...
            aria-label="Open free study material"
          >
          ...
          <button
            ...
            style={{
              color: text,
              background: light ? "var(--bg-ivory)" : "rgba(255,255,255,0.07)",
              border: light
                ? "1px solid var(--border)"
          ...
          {user && (
            <Link
              to={isAdmin ? "/admin" : "/learning-insights"}
              ...
              aria-label="Open learning insights"
            >
              {initials}
            </Link>
          )}
      ...
      {open && (
        <>
          <button
            ...
            className="lg:hidden fixed inset-0 top-16 z-[54] bg-black/45"
          />
          <nav
            id="mobile-navigation"
            aria-label="Mobile navigation"
            className="lg:hidden fixed top-16 left-0 right-0 z-[55] max-h-[calc(100dvh-4rem)] overflow-y-auto overscroll-contain pb-[calc(1.25rem+env(safe-area-inset-bottom))]"
            style={{
              background: surface,
              borderBottom: light
                ? "1px solid var(--border)"
          ...
          >
            <div className="px-4 py-4 space-y-4">
              <Link
                to="/study-material"
                className="mobile-menu-demo-cta flex items-center justify-between gap-3 rounded-2xl px-4 py-4 min-h-16"
              >
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 mobile-menu-demo-icon">
              ...
              {groups.map((group) => {
                ...
                return (
                  <section
                    key={group.title}
                    aria-labelledby={`mobile-${groupId}`}
                    className="mobile-menu-group rounded-2xl p-3"
                    style={{
                      background: light
                        ? "var(--bg-ivory)"
                  ...
                  >
                    <div
                      id={`mobile-${groupId}`}
                      className="px-2 pb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-widest"
                      style={{ color: "var(--gold)" }}
                    >
                      <Icon className="w-4 h-4" />
                      {group.title}
                    </div>
                    <div className="grid grid-cols-1 min-[420px]:grid-cols-2 gap-1">
                      {group.links.map((item) => {
                        ...
                        return (
                          <Link
                            key={item.path}
                            to={item.path}
                            className="min-h-11 rounded-xl px-3 py-2.5 flex items-center justify-between gap-2 text-sm font-semibold"
                            style={{
                              color: active ? "var(--gold)" : text,
                              background: active
                                ? "var(--gold-bg)"
                                : "transparent",
                            }}
                          >
                            ...
                            <ChevronRight
                              ...
                              style={{ color: active ? "var(--gold)" : muted }}
                            />
                          </Link>
                        );
                      })}
              ...
              {user ? (
                <div className="grid gap-2">
                  {isAdmin && (
                    <Link to="/admin" className="btn-primary w-full min-h-12">
                      Owner Panel
                    </Link>
                  )}
                  <Link
                    to="/my-data"
                    className="btn-outline-ink w-full min-h-12"
                  >
                    My Study Data & Backup
                  </Link>
                </div>
              ) : (
                <Link to="/login" className="btn-outline-ink w-full min-h-12">
                  Student Login
                </Link>
              )}
            </div>
