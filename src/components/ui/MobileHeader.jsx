...
import { BookOpen, Brain, ChevronRight, GraduationCap, Menu, X } from 'lucide-react';
...
const groups = [
  { title: 'Learn', icon: BookOpen, links: [
    ...
    { label: 'Courses', path: '/courses' }, { label: 'Lectures', path: '/lectures' },
  ]},
  { title: 'Practice', icon: Brain, links: [
    { label: 'Ask AI Doubt', path: '/ask' }, { label: 'Quizzes', path: '/quizzes' },
    { label: 'Test Series', path: '/test-series' }, { label: 'Games', path: '/games' }, { label: 'Flashcards', path: '/flashcards' },
  ]},
  ...
  return <>
    <header className="mobile-site-header lg:hidden sticky top-0 z-[60] flex items-center justify-between px-4 h-16"
      ...
      <div className="flex items-center gap-2">
        <Link to="/contact" className="mobile-demo-button text-xs font-bold px-3 py-2 rounded-xl whitespace-nowrap" style={{ background: 'linear-gradient(135deg, #C9A050, #B8872F)', color: '#1E1812' }}>Free Demo</Link>
        ...
        <button type="button" onClick={() => setOpen(value => !value)} className="w-11 h-11 rounded-xl flex items-center justify-center"
          style={{ color: text, background: light ? 'var(--bg-ivory)' : 'rgba(255,255,255,0.07)', border: light ? '1px solid var(--border)' : '1px solid rgba(255,255,255,0.1)' }}
    ...
    <AnimatePresence>{open && <>
      ...
      <motion.nav id="mobile-navigation" aria-label="Mobile navigation" initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.2 }}
        ...
        style={{ background: surface, borderBottom: light ? '1px solid var(--border)' : '1px solid rgba(255,255,255,0.1)', boxShadow: '0 20px 50px rgba(0,0,0,0.22)' }}>
