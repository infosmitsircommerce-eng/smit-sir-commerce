...
export default function StudyAccessDialog({ onClose }) {
  ...
  function keyDown(event) {
    ...
    const nodes = [...panel.current.querySelectorAll('a[href],button:not([disabled]),input:not([disabled]),select:not([disabled]),summary')].filter(node => node.getClientRects().length > 0);
