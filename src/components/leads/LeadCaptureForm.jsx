...
export default function LeadCaptureForm({
  ...
  const [form, setForm] = useState({
    fullName: '', mobile: '', parentMobile: '', classLevel: requestedClass, board: requestedBoard, subjects: [requestedSubject],
    studyMode: requestedMode, preferredTime: 'Any time', source: initialSource, demoSubject: requestedSubject,
    message: '', consent: false,
  });

  useEffect(() => {
    trackEvent('lead_form_view', { intent, source: initialSource, landingContext, hasSlot: Boolean(demoSlot?.id) }, user?.id || null);
  ...
  useEffect(() => {
    ...
    setForm((previous) => ({
      ...
      demoSubject: demoSlot.subject_focus && !['Any Commerce Subject', 'Any taught subject'].includes(demoSlot.subject_focus)
        ? demoSlot.subject_focus
        : previous.demoSubject,
    }));
  }, [demoSlot?.id]);
  ...
  const startTracking = () => {
    ...
    trackEvent('lead_form_start', { intent, source: form.source, landingContext, hasSlot: Boolean(demoSlot?.id) }, user?.id || null);
  };
  ...
  const submit = async (event) => {
    ...
    setBusy(true);
    trackEvent('lead_submit_attempt', { intent, source: form.source, landingContext, classLevel: Number(form.classLevel), board: form.board, mode: form.studyMode, hasSlot: Boolean(demoSlot?.id) }, user?.id || null);
    ...
    const payload = {
      ...
      landing_context: landingContext || null,
    };
    ...
    if (submitError) {
      ...
      trackEvent('lead_submit_error', { intent, source: form.source, landingContext, hasSlot: Boolean(demoSlot?.id) }, user?.id || null);
      return;
    }
    ...
    try {
      const emailResponse = await fetch('/api/enquiry-email', {
        ...
        body: JSON.stringify({
          ...
          consent: true,
          landingContext: landingContext || null,
        }),
      });
    ...
    const successMetadata = { intent, source: form.source, landingContext, classLevel: Number(form.classLevel), board: form.board, mode: demoSlot?.mode || form.studyMode, bookedSlot: Boolean(demoSlot?.id) };
    ...
    if (typeof window.gtag === 'function') {
      window.gtag('event', 'generate_lead', {
        ...
        board: form.board,
        study_mode: demoSlot?.mode || form.studyMode,
        booked_slot: Boolean(demoSlot?.id),
        landing_context: landingContext || 'direct-demo',
      });
    }
  ...
  return (
    <form onSubmit={submit} onFocus={startTracking} aria-busy={busy} className={compact ? 'space-y-4' : 'space-y-5'}>
      <div>
        ...
        <p className="text-sm mt-2 leading-relaxed" style={{ color: 'var(--muted)' }}>{demoSlot ? 'Complete your details to reserve the selected slot. Capacity is re-checked when you submit.' : 'Takes about a minute. Tell us your class, board and weak subject. There is no payment and no admission commitment just to request the free paper analysis + demo.'}</p>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        {!compact && <label className="block"><span className="text-xs font-semibold" style={{ color: 'var(--muted)' }}>Parent / guardian mobile <span className="font-normal">(optional)</span></span><input value={form.parentMobile} onChange={set('parentMobile')} className="input-field w-full mt-1.5" placeholder="+91 XXXXX XXXXX" inputMode="tel" autoComplete="tel" /></label>}
        ...
        {!compact && intent === 'Free Demo' && <label className="block"><span className="text-xs font-semibold" style={{ color: 'var(--muted)' }}>Demo subject</span><select value={form.demoSubject} onChange={set('demoSubject')} className="input-field w-full mt-1.5">{DEMO_SUBJECTS.map((subject) => <option key={subject}>{subject}</option>)}</select></label>}
      </div>

      {!compact && <fieldset>
        <legend className="text-xs font-semibold mb-2" style={{ color: 'var(--muted)' }}>Subjects you need help with *</legend>
        <div className="flex flex-wrap gap-2">
          {SUBJECTS.map((subject) => {
            const active = form.subjects.includes(subject);
            return <button type="button" aria-pressed={active} key={subject} onClick={() => toggleSubject(subject)} className="inline-flex items-center gap-1.5 rounded-full px-3 py-2 text-xs font-semibold transition-all" style={{ background: active ? 'var(--gold-bg)' : 'var(--bg-ivory)', border: active ? '1px solid rgba(184,135,47,.45)' : '1px solid var(--border)', color: 'var(--charcoal)' }}>{active && <Check className="w-3.5 h-3.5" />}{subject}</button>;
          })}
        </div>
      </fieldset>}

      {!compact && <div className="grid sm:grid-cols-2 gap-4">
      ...
      {compact && <details className="rounded-xl p-3" style={{ background: 'var(--bg-ivory)', border: '1px solid var(--border)' }}>
        <summary className="cursor-pointer text-sm font-semibold" style={{ color: 'var(--ink)' }}>Add study preferences <span className="font-normal" style={{ color: 'var(--muted)' }}>(optional)</span></summary>
        <div className="grid sm:grid-cols-2 gap-4 mt-4">
          ...
          <label className="block sm:col-span-2"><span className="text-xs font-semibold" style={{ color: 'var(--muted)' }}>Parent / guardian mobile <span className="font-normal">(optional)</span></span><input value={form.parentMobile} onChange={set('parentMobile')} className="input-field w-full mt-1.5" placeholder="+91 XXXXX XXXXX" inputMode="tel" autoComplete="tel" /></label>
          <label className="block sm:col-span-2"><span className="text-xs font-semibold" style={{ color: 'var(--muted)' }}>Anything specific? <span className="font-normal">(optional)</span></span><textarea maxLength={1000} rows={2} value={form.message} onChange={set('message')} className="input-field w-full mt-1.5 resize-none" placeholder="Example: National Income numericals" /></label>
        </div>
      </details>}
      ...
      {error && <div role="alert" aria-live="polite" className="rounded-xl p-3 text-sm" style={{ background: 'rgba(180,83,60,.08)', border: '1px solid rgba(180,83,60,.2)', color: '#B4533C' }}>{error}</div>}

      <div className="grid grid-cols-3 gap-2 text-center text-[11px] font-semibold" style={{ color: 'var(--muted)' }}>
        <div className="tile-paper px-2 py-2">No payment</div>
        <div className="tile-paper px-2 py-2">No admission pressure</div>
        <div className="tile-paper px-2 py-2">Mehsana + online</div>
      </div>

      <button type="submit" disabled={busy} className="btn-primary w-full inline-flex items-center justify-center gap-2 py-3.5">{busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}{busy ? 'Checking availability…' : demoSlot ? 'Reserve This Demo Slot' : intent === 'Free Demo' ? 'Get My Free Paper Analysis' : 'Send Enquiry'}</button>
