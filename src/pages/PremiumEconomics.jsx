...
export default function PremiumEconomics() {
  ...
  useEffect(() => { setGuide(null); setError(''); }, [user?.id, access]);
  ...
  if ((!user || !access) && group !== 'gseb') return <main className="page-container py-16 max-w-xl"><SEO title="Premium Economics Library" description="Protected Premium Economics study guides." path="/premium/economics" noindex /><div className="card-paper p-8 text-center"><LockKeyhole className="w-12 h-12 mx-auto" style={{ color: 'var(--gold)' }} /><h1 className="text-3xl mt-5">Premium Economics Library</h1><p className="mt-3">CBSE Economics deep-dives and GSEB Class 12 Economics revision PDFs.</p><p className="mt-4 leading-7">{!user ? 'Sign in with your registered student account to check access.' : 'Your account currently has Free access. Premium costs ₹999 once and unlocks this library plus Hard and Extreme quizzes.'}</p><Link to={!user ? '/login' : '/quizzes'} className="btn-primary inline-flex mt-6">{!user ? 'Sign in' : 'View ₹999 Premium access'}</Link><a href="/gseb-class-12-economics.html" className="btn-secondary inline-flex mt-4">Browse free GSEB notes</a></div></main>;
  ...
  return <main className="page-container py-10 max-w-6xl"><SEO title="Premium Economics Library" description="Protected Premium Economics study guides." path="/premium/economics" noindex /><span className="eyebrow">Premium learning library</span><h1 className="text-4xl mt-4">Economics Premium library</h1><p className="mt-4 max-w-3xl leading-7">31 chapters with 20 concept explanations and 20 worked Hard/Extreme challenges per chapter. Also includes 10 GSEB Class 12 Economics revision PDFs. Browse the chapter list below. Premium accounts can open the resources; Free accounts can view the payment QR.</p><div className="flex flex-wrap gap-2 mt-6">{[['micro', 'Class 11 Micro'], ['macro', 'Class 12 Macro'], ['ied', 'Class 12 IED'], ['gseb', 'GSEB Class 12 PDFs']].map(([id, label]) => <button key={id} onClick={() => setGroup(id)} className={group === id ? 'btn-primary' : 'btn-secondary'}>{label}</button>)}</div>{error && <p role="alert" className="mt-4 text-red-700">{error}</p>}{group === 'gseb' ? <PremiumGsebEconomics /> : <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4 mt-7">{packs.map((pack) => <article key={pack.id} className="card-paper p-5"><BookOpenCheck className="w-6 h-6" style={{ color: 'var(--gold)' }} /><h2 className="text-xl mt-3">{pack.title}</h2><p className="text-sm mt-2">20 concepts · 20 worked challenges</p><button onClick={() => open(pack)} disabled={busy === pack.id} className="btn-primary w-full mt-5">{busy === pack.id ? 'Loading…' : 'Open Premium guide'}</button></article>)}</div>}</main>;
}
