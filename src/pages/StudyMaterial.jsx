...
function MaterialCard({ material }) {
  ...
  return (
    <motion.article whileHover={{ y: -3 }} className="card-paper group flex flex-col p-5">
      ...
      <div className="flex items-center gap-2 text-xs mb-4 flex-wrap" style={{ color: 'var(--subtle)' }}>
        <span>{material.subject}</span><span>•</span><span>Class {classLevel}</span>
        {material.pages && <><span>•</span><span>{material.pages} pages</span></>}
      </div>

      <div className="mt-auto flex gap-2">
        {viewUrl && (
          <a href={viewUrl} className="tile-paper flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs font-semibold" style={{ color: 'var(--charcoal)' }}>
            <Eye className="w-3.5 h-3.5" /> View notes
          </a>
        )}
        {fileUrl && (
          <a href={fileUrl} download target="_blank" rel="noopener noreferrer" className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-xs font-semibold transition-all" style={{ background: 'var(--gold-bg)', border: '1px solid rgba(184,135,47,0.3)', color: 'var(--gold)' }}>
    ...
    <div className="min-h-screen" style={{ background: 'var(--bg-ivory)' }}>
      <div className="page-hero">
        <div className="page-container text-center">
          <span className="eyebrow">CBSE + GSEB Study Resources</span>
          <h1 className="mt-5">Study <em>material.</em></h1>
          <p className="mx-auto">Choose your board, subject and chapter. CBSE resources stay separate from the new GSEB Class 12 Economics collection.</p>
          <div className="mt-6 inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-medium" style={{ background: 'rgba(77,124,15,0.08)', border: '1px solid rgba(77,124,15,0.25)', color: 'var(--green)' }}>✓ Published study resources are free to open</div>
        </div>
      </div>

      <div className="page-container section-padding">
        <section className="mb-12" aria-labelledby="collections-title">
          <div className="text-center max-w-3xl mx-auto mb-7">
            <span className="eyebrow">Start with your subject</span>
            <h2 id="collections-title" className="text-3xl mt-3" style={{ fontFamily: 'var(--font-serif)', color: 'var(--ink)' }}>Free chapter-wise commerce notes</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <a href="/gseb-class-12-economics.html" className="card-paper p-6 group" style={{ borderColor: 'rgba(184,135,47,0.35)' }}>
              <div className="text-xs font-semibold mb-2" style={{ color: 'var(--gold)' }}>GSEB • CLASS 12</div>
              <h3 className="text-xl mb-2" style={{ fontFamily: 'var(--font-serif)', color: 'var(--ink)' }}>Class 12 Economics Notes</h3>
              <p className="text-sm mb-4" style={{ color: 'var(--muted)' }}>10 chapters available • Chapters 2–11</p>
              <span className="text-sm font-semibold" style={{ color: 'var(--gold)' }}>Browse GSEB collection →</span>
            </a>
          </div>
        </section>
        ...
        <div className="text-center mb-10">
          ...
          <div className="toggle-paper inline-flex max-w-full" role="group" aria-label="Choose board">
            <button onClick={() => selectBoard('CBSE')} className={filterBoard === 'CBSE' ? 'active' : ''}>CBSE Board</button>
            <button onClick={() => selectBoard('GSEB')} className={filterBoard === 'GSEB' ? 'active' : ''}>GSEB Board</button>
          </div>
          {filterBoard === 'GSEB' && <p className="text-xs mt-3" style={{ color: 'var(--muted)' }}>Class 12 Economics Chapters 2–11 are available. Chapter 1 has not been uploaded.</p>}
        </div>
        ...
        <div className="material-type-pills flex flex-nowrap sm:flex-wrap gap-2 mb-8 overflow-x-auto scrollbar-hide pb-2" aria-label="Material types">
          {visibleTypes.map((type) => (
            <button key={type} onClick={() => setFilterType(type)} className="px-3 py-1.5 rounded-full text-xs font-medium transition-colors whitespace-nowrap" style={pillStyle(filterType === type)}>{type === 'All' ? 'All Types' : type}</button>
          ))}
        </div>
        ...
        <div className="card-paper p-4 sm:p-5 mb-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[minmax(260px,1fr)_auto_auto] gap-3 sm:gap-4">
          <div className="relative sm:col-span-2 lg:col-span-1">
            ...
            <input style={{ ...inputStyle, paddingLeft: '40px' }} placeholder={`Search ${filterBoard} material...`} value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
          <select value={filterClass} onChange={(e) => setFilterClass(e.target.value)} style={inputStyle}>
            <option value="All">All Classes</option><option value="11">Class 11</option><option value="12">Class 12</option>
          </select>
        ...
        <div className="text-sm mb-5" style={{ color: 'var(--muted)' }}>Showing <span className="font-semibold" style={{ color: 'var(--gold)' }}>{filtered.length}</span> {filterBoard} materials</div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">{filtered.map((material) => <MaterialCard key={material.id} material={material} />)}</div>
