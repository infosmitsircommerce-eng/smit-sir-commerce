...
export default function LeadCRM() {
  ...
  const visible = useMemo(() => {
    ...
    return leads.filter((lead) => {
      ...
      if (!q) return true;
      return `${lead.full_name || ''} ${lead.mobile || ''} ${lead.board || ''} ${(lead.subjects || []).join(' ')} ${lead.study_mode || ''} ${lead.message || ''} ${lead.landing_context || ''} ${lead.first_path || ''}`.toLowerCase().includes(q);
    });
  ...
  const patchLead = async (leadId, patch, successText = 'Lead updated.') => {
    ...
    if (error) setMessage(`Could not update lead: ${error.message}`);
      ...
      if (patch.status === 'Joined') {
        ...
        if (typeof window.gtag === 'function') window.gtag('event', 'lead_joined', joinedMetadata);
      }
  ...
  const exportLeads = () => {
    const header = ['Name','Mobile','Class','Board','Subjects','Mode','Preferred contact','Source','Landing context','First page','Intent','Status','Enquiries','Next follow-up','Last enquiry','Notes'];
    const rows = leads.map((lead) => [lead.full_name, lead.mobile, lead.class_level, lead.board, lead.subjects, lead.study_mode, lead.preferred_contact_time, lead.source, lead.landing_context, lead.first_path, lead.intent, lead.status, lead.enquiries_count, lead.next_follow_up, lead.last_enquiry_at, lead.owner_notes]);
