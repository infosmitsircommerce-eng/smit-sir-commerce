...
import { BookOpenCheck, CalendarCheck2, CalendarClock, Loader2, Megaphone, ShieldCheck, UploadCloud, Users } from 'lucide-react';
...
export default function OwnerHome() {
  ...
  return <div className="min-h-screen" style={{ background: 'var(--bg-ivory)' }}><SEO title="Owner Hub" description="Restricted owner administration hub." path="/admin" noindex /><section className="page-hero"><div className="page-container max-w-5xl"><span className="eyebrow">Owner control</span><h1 className="mt-4">Run the platform from <em>one place.</em></h1><p className="mt-4 max-w-2xl" style={{ color: 'var(--muted)' }}>Only real account, admissions, demo and content data appears here. No demo students or fake business statistics.</p></div></section><main className="page-container pb-16"><div className="grid md:grid-cols-2 xl:grid-cols-5 gap-5">{cards.map(({ to, icon: Icon, title, value, text }) => <Link key={to} to={to} className="card-paper p-6 group"><div className="w-11 h-11 rounded-xl flex items-center justify-center" style={{ background: 'var(--gold-bg)', color: 'var(--gold)' }}><Icon className="w-5 h-5" /></div><div className="text-4xl font-bold mt-5" style={{ fontFamily: 'var(--font-serif)', color: 'var(--ink)' }}>{value}</div><h2 className="text-xl font-bold mt-2" style={{ color: 'var(--ink)' }}>{title}</h2><p className="text-sm mt-2 leading-relaxed" style={{ color: 'var(--muted)' }}>{text}</p><div className="text-sm font-bold mt-5" style={{ color: 'var(--gold)' }}>Open →</div></Link>)}</div><div className="mt-6 grid md:grid-cols-[1fr_auto] gap-4 items-center card-paper p-5" style={{ border: '1px solid rgba(184,135,47,.24)', background: 'linear-gradient(135deg,#fff9e7,#fff)' }}><div className="flex gap-4 items-start"><span className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0" style={{ background: '#17284b', color: '#f4d486' }}><UploadCloud className="w-5 h-5" /></span><div><strong style={{ color: '#17284b' }}>BST Premium PDF Sync</strong><p className="text-sm mt-1 leading-relaxed" style={{ color: 'var(--muted)' }}>Securely upload all 12 CBSE Class 12 Business Studies Detailed Premium Masters to protected storage.</p></div></div><a href="/admin-cbse12-business-studies-upload.html" className="btn-primary text-center">Open bulk uploader</a></div><div className="mt-6 flex justify-end"><Link to="/" className="btn-secondary">View public site</Link></div></main></div>;
}
