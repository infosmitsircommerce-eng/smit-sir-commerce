    ...
    <main className="page-container section-padding"><div className="grid lg:grid-cols-[minmax(0,1fr)_300px] gap-8 items-start">
      <div className="space-y-7">
        ...
        {guide.sections.map((section, sectionIndex) => <section key={section.title} className="card-paper p-5 sm:p-8">
          ...
          {section.items?.length > 0 && <ol className="mt-5 space-y-2">{section.items.map((item, itemIndex) => <li key={`${section.title}-${itemIndex}`} className="tile-paper p-4 flex gap-3 text-sm leading-7"><span className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-black flex-shrink-0" style={{ background: 'var(--gold-bg)', color: 'var(--gold)' }}>{itemIndex + 1}</span><span style={{ color: 'var(--charcoal)' }}>{item}</span></li>)}</ol>}
