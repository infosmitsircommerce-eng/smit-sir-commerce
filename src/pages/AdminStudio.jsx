...
import PremiumConversionAnalytics from '../components/admin/PremiumConversionAnalytics';
...
export default function AdminStudio() {
  ...
  const [tab, setTab] = useState(() => new URLSearchParams(window.location.search).get('tab') === 'analytics' ? 'analytics' : 'content');
  const [form, setForm] = useState(emptyTest);
  const [items, setItems] = useState([]);
  const [events, setEvents] = useState([]);
  const [database, setDatabase] = useState({ content_items: 'checking', learning_events: 'checking', student_learning_state: 'checking', test_attempts: 'checking', commerce_materials: 'checking' });
  ...
  const refresh = async () => {
    ...
    const checks = await Promise.all([
      ...
      supabase.storage.from('commerce-materials').list('', { limit: 1 }),
    ]);
    setDatabase({
      ...
      commerce_materials: checks[4].error ? 'missing' : 'ready',
    });
    if (!checks[0].error) {
  ...
  const analytics = useMemo(() => {
    ...
    let setupCompletions = 0; let searches = 0; let resourceOpens = 0; let pdfOpens = 0; let appInstalls = 0;
    events.forEach((event) => {
      ...
      if (event.event_name === 'student_setup_complete') setupCompletions += 1;
      if (event.event_name === 'global_search_submit') searches += 1;
      if (event.event_name === 'pwa_install_accept') appInstalls += 1;
      if (event.event_name === 'mobile_quick_action_click') {
        const action = event.metadata?.action || 'Unknown action';
        actionCounts.set(action, (actionCounts.get(action) || 0) + 1);
      }
      if (event.event_name === 'resource_open') {
        resourceOpens += 1;
        if (String(event.metadata?.action || '').startsWith('pdf_')) pdfOpens += 1;
        const resource = event.metadata?.resourceId || event.path || 'Unknown resource';
    ...
    return {
      ...
      topResources: [...resourceCounts.entries()].sort((a, b) => b[1] - a[1]).slice(0, 10),
  ...
  return <div className="min-h-screen" style={{ background: '#0f0d2e' }}>
    ...
    <main className="page-container py-8">
      ...
      {tab === 'analytics' && <><PremiumConversionAnalytics /><div className="grid grid-cols-2 lg:grid-cols-4 gap-4">{[['Events',analytics.total],['Exam starts',analytics.examStarts],['Exam completions',analytics.examCompletes],['Exam completion rate',`${analytics.examCompletionRate}%`]].map(([label,value]) => <div key={label} className="card-premium"><Activity className="w-5 h-5 text-gold-400" /><div className="text-3xl font-black text-white mt-3">{value}</div><div className="text-xs text-navy-400 mt-1">{label}</div></div>)}</div><div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mt-4">{[['Study setups',analytics.setupCompletions],['Searches',analytics.searches],['Resources opened',analytics.resourceOpens],['PDF actions',analytics.pdfOpens],['App installs',analytics.appInstalls]].map(([label,value]) => <div key={label} className="card-premium"><div className="text-2xl font-black text-white">{value}</div><div className="text-xs text-navy-400 mt-1">{label}</div></div>)}</div>{database.learning_events !== 'ready' ? <div className="card-premium mt-6 text-red-300"><AlertCircle className="w-5 h-5 inline mr-2" /> Analytics database table is not active yet.</div> : <div className="grid lg:grid-cols-2 gap-6 mt-6"><section className="card-premium"><h2 className="text-lg font-bold text-white flex items-center gap-2"><BarChart3 className="w-5 h-5 text-gold-400" /> Popular study resources</h2><div className="space-y-2 mt-4">{analytics.topResources.length ? analytics.topResources.map(([name,count]) => <div key={name} className="flex justify-between gap-4 p-3 rounded-xl bg-white/5 text-sm"><span className="text-navy-300 truncate">{name}</span><strong className="text-white">{count}</strong></div>) : <p className="text-sm text-navy-400">New resource-open data will appear as students use the finder.</p>}</div></section><section className="card-premium"><h2 className="text-lg font-bold text-white">Most-used home shortcuts</h2><div className="space-y-2 mt-4">{analytics.topActions.length ? analytics.topActions.map(([name,count]) => <div key={name} className="flex justify-between gap-4 p-3 rounded-xl bg-white/5 text-sm"><span className="text-navy-300">{name}</span><strong className="text-white">{count}</strong></div>) : <p className="text-sm text-navy-400">Shortcut usage will appear after the new mobile dashboard is used.</p>}</div></section><section className="card-premium"><h2 className="text-lg font-bold text-white flex items-center gap-2"><Activity className="w-5 h-5 text-gold-400" /> Top events</h2><div className="space-y-2 mt-4">{analytics.topEvents.map(([name,count]) => <div key={name} className="flex justify-between p-3 rounded-xl bg-white/5 text-sm"><span className="text-navy-300">{name}</span><strong className="text-white">{count}</strong></div>)}</div></section><section className="card-premium"><h2 className="text-lg font-bold text-white">Most viewed pages</h2><div className="space-y-2 mt-4">{analytics.topPages.map(([path,count]) => <div key={path} className="flex justify-between gap-4 p-3 rounded-xl bg-white/5 text-sm"><span className="text-navy-300 truncate">{path}</span><strong className="text-white">{count}</strong></div>)}</div></section></div>}</>}
      {tab === 'database' && <section className="max-w-3xl mx-auto"><div className="card-premium"><Database className="w-8 h-8 text-gold-400" /><h2 className="text-2xl font-bold text-white mt-4">Platform database readiness</h2><p className="text-sm text-navy-400 mt-2">This checks the live Supabase project, including the public Commerce PDF bucket used by the Resource Engine.</p><div className="space-y-3 mt-6">{Object.entries(database).map(([table,status]) => <div key={table} className="flex items-center justify-between p-4 rounded-xl bg-white/5"><code className="text-white">{table}</code><span className={`text-xs font-bold inline-flex items-center gap-1 ${status === 'ready' ? 'text-emerald-400' : status === 'checking' ? 'text-gold-400' : 'text-red-400'}`}>{status === 'ready' ? <CheckCircle2 className="w-4 h-4" /> : status === 'checking' ? <Loader2 className="w-4 h-4 animate-spin" /> : <AlertCircle className="w-4 h-4" />}{status}</span></div>)}</div><div className="mt-6 p-4 rounded-xl bg-gold-500/10 text-gold-200 text-sm leading-relaxed">Core platform tables use <code>supabase/platform_hardening.sql</code>. The Commerce upload bucket and admin-only storage policies use <code>supabase/commerce_resource_engine.sql</code>. Both are represented in the readiness checks above.</div><button onClick={refresh} className="btn-primary mt-5 inline-flex items-center gap-2"><Database className="w-4 h-4" /> Re-check database</button></div></section>}
    </main>
  </div>;
}
