...
function ProductCard({ product, selected, onSelect }) {
  return (
    <article className="card-paper p-5 sm:p-7 flex flex-col" style={selected ? { outline: "2px solid rgba(184,135,47,.45)", boxShadow: "0 18px 45px rgba(76,55,20,.10)" } : undefined}>
      ...
      <details className="mt-5 rounded-xl" style={{ border: "1px solid var(--border)", background: "#fff" }}>
        <summary className="cursor-pointer p-4 font-black text-sm">See exactly what is included</summary>
      ...
      <div className="mt-5 grid gap-2">
        <Link to={product.previewPath} onClick={() => trackEvent("board_booster_preview_click", { productId: product.id })} className="btn-secondary w-full inline-flex items-center justify-center gap-2">
          <Eye className="w-4 h-4" /> View free preview
        </Link>
    ...
    <main className="min-h-screen pb-28 lg:pb-0" style={{ background: "var(--bg-ivory)", color: "var(--ink)" }}>
      ...
      <section className="section-padding" style={{ background: "linear-gradient(145deg,#0f172a 0%,#16233f 54%,#2e2618 100%)", color: "#fff" }}>
        <div className="page-container grid lg:grid-cols-[1.08fr_.92fr] gap-8 lg:gap-12 items-center">
          <div>
            ...
            <p className="text-base sm:text-lg leading-8 mt-5 max-w-2xl" style={{ color: "#d2d9e6" }}>Free notes stay free. Premium packs add deeper explanations, harder practice and focused revision. Every pack below shows its real resource count and a free preview first.</p>
          ...
          <div className="rounded-3xl p-5 sm:p-7" style={{ background: "rgba(255,255,255,.96)", color: "var(--ink)", boxShadow: "0 30px 70px rgba(0,0,0,.28)" }}>
            ...
            <Link to={BOARD_BOOSTER_PRODUCTS[0].previewPath} className="btn-secondary w-full mt-5 inline-flex items-center justify-center gap-2"><Eye className="w-4 h-4" /> Preview free material</Link>
      ...
      <section id="catalog" className="page-container section-padding">
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-5">
          <div><span className="eyebrow">Organized catalogue</span><h2 className="text-3xl sm:text-5xl mt-3" style={{ fontFamily: "var(--font-serif)" }}>Board → Class → Subject → Pack</h2><p className="mt-3 max-w-2xl leading-7" style={{ color: "var(--muted)" }}>No random PDF wall. Filter once and see the exact pack or free collection you need.</p></div>
        ...
        {view === "premium" ? (
          ...
          <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4 mt-7" id="free-library">{filteredFree.map((collection) => <article key={collection.id} className="card-paper p-5 sm:p-6"><div className="flex items-center justify-between gap-3"><span className="eyebrow">{collection.board} · Class {collection.classLevel}</span><span className="rounded-full px-3 py-1 text-xs font-black" style={{ background: "#edf9f0", color: "#21663a" }}>FREE</span></div><h3 className="text-xl font-black mt-4">{collection.subject}</h3><div className="text-2xl font-black mt-3" style={{ color: "var(--gold)" }}>{collection.count}</div><p className="text-sm leading-6 mt-3" style={{ color: "var(--muted)" }}>{collection.note}</p><Link to={collection.path} className="btn-secondary w-full mt-5 inline-flex items-center justify-center gap-2">Open free resources <ChevronRight className="w-4 h-4" /></Link></article>)}</div>
        )}
      </section>
      ...
      <section className="section-padding" style={{ background: "#fff" }}>
        <div className="page-container grid lg:grid-cols-[1fr_.9fr] gap-8 items-start">
          <div>
            ...
            <p className="mt-4 max-w-2xl leading-7" style={{ color: "var(--muted)" }}>Every paid pack now has a defined board, class, subject, price, real inventory and preview route. Collections that are currently just published notes stay free.</p>
