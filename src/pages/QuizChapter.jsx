...
import { getRelatedQuizPages, quizPageByPath } from '../data/quizDiscovery';
...
export default function QuizChapter() {
  ...
  return <main className="page-container py-10 max-w-5xl ssc-study-access ssc-quiz-chapter my-6">
    ...
    <h1 className="text-3xl sm:text-4xl" style={{color: 'var(--study-ink)', fontFamily: 'var(--font-sans, sans-serif)'}}>{page.title}</h1>
    <p className="mt-4 text-base leading-7">Test your understanding of {pack.title} with 20 free questions across Easy and {page.freeLevelLabel}. Choose a level below, attempt each question, then review the explanation before moving on. Hard and Extreme require Premium.</p>
    <a className="ssc-study-pdf mt-4" href={'https://wa.me/?text='+encodeURIComponent('Practise '+page.title+' with free Easy and '+page.freeLevelLabel+' questions: '+base+page.path)} target="_blank" rel="noopener noreferrer">Share this chapter quiz</a>
    <div className="flex flex-wrap gap-3 mt-4">
      <Link className="ssc-study-open" to={page.testPath}>Open this chapter test</Link>
      {page.notesPath && <a className="underline self-center" href={page.notesPath}>Read {pack.title} notes</a>}
    </div>
    <p className="text-sm leading-6 mt-4">{pack.sourceLabel}</p>
    <LevelGrid key={pack.id} pack={pack} appearance="study" />
    <section className="mt-10"><h2 className="text-2xl">Key concepts to revise</h2><div className="grid sm:grid-cols-2 gap-4 mt-5">{pack.levels.Easy.map((q,i)=><article key={i} className="rounded-xl border p-4"><h3 className="font-bold">{q.q.match(/\"(.*?)\"/)?.[1] || q.q}</h3><p className="text-base leading-7 mt-2">{q.explanation}</p></article>)}</div></section>
